...
class BatchRanker:
    ...
    def rerank(
        ...
        top_trials = random.sample(ranking, k=self.num_batch)
        remaining_trials = [trial for trial in ranking if trial not in top_trials]
        batch_size = len(remaining_trials) // self.num_batch
