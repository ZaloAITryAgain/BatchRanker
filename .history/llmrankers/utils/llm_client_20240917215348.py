...
from service_platform.service.ai.llms.prompt import BasePromptFunction
from service_platform.exception.server_error import ServerError
from service_platform.service.ai.llms.llm_schema import (
    ChatMessage,
    MessageRole,
    ParseType,
)
...
class LLMClient:
    def __init__(self):
        self.chat_model = settings.llm.azure_openai.model
        self.embed_model = settings.llm.azure_openai.embed_model
        self._patch_structure_completion()

    @classmethod
    async def setup(cls, app: FastAPI):
        # add client for partial steering (aka streaming with structured output - no validation)
        # Only use OpenAI for now, refer to https://python.useinstructor.com/hub/partial_streaming/
        key = settings.llm.azure_openai.api_key or LLMClient._openai_key()
        cls._openai_client = await app.state.acontext_stack.enter_async_context(
            AsyncOpenAI(api_key=key),
        )
        cls.client = instructor.from_openai(client=cls._openai_client)
