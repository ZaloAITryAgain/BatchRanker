...
class PatientSummaryInfo(CoreModel):
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "PatientSummaryInfo":
        return cls(**data)


class PatientKeywordInfo(CoreModel):
    criteria: dict

    @classmethod
    def from_dict(cls, data: dict) -> "PatientKeywordInfo":
        return cls(**data)
...
class TrialScoringBatchResponse(CoreModel):
    ...
    trial_scores: Optional[list[TrialScoringResponse]] = Field(
        description="The list of scoring results",
    )

class TrialScoringBatchNoCOTResponse(CoreModel):
