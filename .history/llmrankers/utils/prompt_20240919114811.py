...
class SetwiseRankRelevancePrompt(BasePromptFunction):
    ...
    def system(self):
        prompt = (
            ...
            "A score of 0 means the document is the least relevant among all documents for this query, 80 might mean that it could be the most relevant among the documents but not exactly relevant to the query, while 100 means it's the most relevant relative to other documents and exactly to the query.\n"
        )
        if self.use_COT:
            prompt += (
