...
class TourwiseLlmRanker(LlmRanker):
    def __init__(
        self,
...
Document 3, ..., Document 1"""
    ...
    def get_top_M(self, answer: str, N: int = 10, M: int = 5, groups_docid: List[str] = []) -> List[str]:
    ...
    def filter_processing(self, y_it: int, query: str, docs_id: List[str], all_contents: Dict[str, str], docs_score_dicts_list: List) -> None:
        ...
        with Manager() as manager:
            ...
            for groups in docs_groups:
                p = Process(target=self.group_processing, args=(groups, query, N, M, all_contents, groups_score_dict_list))
                p.start()
                processes.append(p)
            for p in processes:
                p.join()
        ...
        ranked_list = self.sort_docs_by_relevance(list(docs_score_dict.keys()), list(docs_score_dict.values()))
    ...
    def group_processing(self, groups: List[str], query: str, N: int, M: int, all_contents: Dict[str, str], groups_score_dict_list: List) -> None:
    ...
    def get_groups_skip(self, docs_id: List[str], to_n_groups: int = 10, m_docs_per_group: int = 10) -> List[List[str]]:
    ...
    def sort_docs_by_relevance(self, doc_ids: List[str], relevance_scores: List[float]) -> List[str]:
    ...
    def rerank(self, query: str, ranking: List[SearchResult]) -> List[SearchResult]:
        ...
        with Manager() as manager:
            ...
            for y in range(self.num_tournaments):
                p = Process(target=self.filter_processing, args=(y, query, docs_id, all_contents, docs_score_dicts_list))
                p.start()
                processes.append(p)
            for p in processes:
                p.join()
        ...
        ranked_list = self.sort_docs_by_relevance(list(docs_score_dict.keys()), list(docs_score_dict.values()))
        return [SearchResult(docid=doc_id, score=docs_score_dict[doc_id], text=all_contents[doc_id]) 
